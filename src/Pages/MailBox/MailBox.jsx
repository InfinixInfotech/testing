...
const MailBox = () => {
  ...
  useEffect(() => {
    dispatch(getMailByEmpIdThunk("INFSASHANT1007"));
  }, [dispatch]);
    ...
    <div className="container" style={{ marginTop: "70px" }}>
      {allData.map((email) => (
        <div
          ...
          className="card mb-2"
          style={{
            backgroundColor: isNewMail(new Date(email.createDate).getTime()) ? "#FAF9F6" : "white",
          }}
        >
          ...
          >
            {(email.subject && email.subject.split("<br>").length > 2) ? email.subject.split("<br>")[2] : "Tag"}
          </span>
          ...
          <div className="card-body d-flex justify-content-between align-items-center mt-4">
            ...
            <Link to={`/email/${email.id}`} className="text-decoration-none text-dark">
              <div>
                ...
                <h5 className="card-title">{email?.subject?.split("<br>")[0] || "No Subject"}</h5>
